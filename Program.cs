...
namespace Test_of_increace
{
    class Program
    {
        ...
        static void BuildAll(int size, string[,] hitMatrix, string[,] selectMatrix, int crosses, int numberOfboats, int hitsUsed)
        {
            ...
            for (int b = 0; b < (size); b++)
            {
                ...
                for (int c = 0; c < size; c++)
                {
                    ...
                    Console.Write(displayMatrix[c, b]);
            ...
            int remainingHits = 0; 
            remainingHits = (((size * size) - total) / 2) - hitsUsed;
        ...
        static void Main(string[] args)
        {
            ...
            while (isRunning)
            {
                numberofboats = safeIntInput("Please enter desired number of boats: ");
                if (numberofboats > size - 1)
                {
                    Console.WriteLine("Too many boats!");
                }
                else
                {
                    isRunning = false;
